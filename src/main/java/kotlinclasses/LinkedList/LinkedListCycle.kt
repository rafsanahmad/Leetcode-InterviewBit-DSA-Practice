 ...
 *  * Created by Rafsan Ahmad on 5/26/25, 9:19PM
...
index of the node that tail's next pointer is connected to. Note that pos is not passed as a
parameter.
...
fun main() {
    ...
    listNode.next?.next?.next = temp
    println(linkedListCycle.hasCycle(listNode))
}