/*
 * *
 *  * Snakes and Ladders.kt
 *  * Created by Rafsan Ahmad on 8/5/25, 7:23PM
...
that snake or ladder is board[r][c]. Squares 1 and n2 are not the starting points of any snake or
ladder.
...
Return the least number of dice rolls required to reach the square n2. If it is not possible to reach
the square, return -1.
...
The squares labeled 1 and n2 are not the starting points of any snake or ladder.*/
    ...
    fun snakesAndLadders(board: Array<IntArray>): Int {
        val n = board.size
        val cells = IntArray(n * n + 1)  // 1-based index
        var index = 1
        var leftToRight = true

        // Flatten 2D board to 1D (cells[1..n^2])
        ...
        while (queue.isNotEmpty()) {
            val (pos, moves) = queue.poll()

            if (pos == n * n) return moves

            for (next in pos + 1..minOf(pos + 6, n * n)) {
                val dest = if (cells[next] == -1) next else cells[next]
                if (!visited[dest]) {
                    visited[dest] = true
                    queue.offer(dest to moves + 1)
                }
            }
        }
...
fun main() {
    ...
    println(obj.snakesAndLadders(board))
}