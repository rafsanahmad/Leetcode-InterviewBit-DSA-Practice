/*
 * *
 *  * Implementing Water Supply Problem using Breadth First Search.java
 *  * Created by Rafsan Ahmad on 7/12/22, 11:47 PM
...
public class WaterSupplyRoute {
    /*Given N cities that are connected using N-1 roads. Between Cities [i, i+1], there exists an edge for all i from
    1 to N-1.
...
We run a breadth-first search on each city and check for two things: The city is not blocked and the city is not visited.
 If both these conditions return true then we run a breadth-first search from that city and count the number of cities
 up to which water can be supplied.
