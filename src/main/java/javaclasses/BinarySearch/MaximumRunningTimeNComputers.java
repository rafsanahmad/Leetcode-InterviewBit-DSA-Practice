/*
 * *
 *  * Maximum Running Time of N Computers.java
 *  * Created by Rafsan Ahmad on 7/30/23, 8:32 PM
...
Initially, you can insert at most one battery into each computer. After that and at any integer
 ...
 times. The inserted battery can be a totally new battery or a battery from another computer.
...
After one minute, battery 0 and battery 2 are drained so you need to remove them and insert
battery 1 into the first computer and battery 3 into the second computer.
After another minute, battery 1 and battery 3 are also drained so the first and second computers
...
1 <= batteries[i] <= 10^9*/
    ...
    public boolean checkIfRun(int n, long mid, int[] batteries) {
        long target = n * mid;
        long total = 0;
    ...
    public static void main(String[] args) {
        MaximumRunningTimeNComputers nComputers = new MaximumRunningTimeNComputers();
        int[] arr = {3, 3, 3};
        System.out.println(nComputers.maxRunTime(2, arr));
    }
}
